<template>
  <div id="changeemail" class="bg-main">
    ...
    <el-form>
      ...
      <el-form-item :label="$t('register.codenum')">
        <el-input v-model="pms.phone_code">
          <template slot="append">
            <el-button
              :disabled="phone == ''||front == '' || phoneTip !=$t('register.sendcode')"
      ...
      <el-form-item :label="$t('change.newEmail')">
        <el-input v-model="pms.new_email"></el-input>
      </el-form-item>
      <el-form-item :label="$t('register.codenum')">
        <el-input v-model="pms.email_code">
          <template slot="append">
            <el-button :disabled="pms.phone_code == '' || emailTip !=$t('register.sendcode')" @click="sendCode('email')">{{emailTip}}</el-button>
          </template>
        </el-input>
      </el-form-item>
      <el-form-item>
  ...
  methods: {
    ...
    sendCode(which) {
      ...
      if (which == "email") {
        ...
        if(typeof (this.phone-0) == 'number'){
          data = {number:this.phone,front:this.front,type:'email'};
          url = 'sms_amend';
        } else {
