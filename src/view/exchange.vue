  ...
  <div class="exchange clr-part flex column">
    ...
    <div class="content bg-part mb10 flex1">
      ...
      <ul class="list-item ft12 tc">
        <li
          :class="['curPer','redColor','bg-hov',{'bg-evev':index%2 != 0}]"
          v-for="(out,index) in outlist"
          @click="price(out.price)"
        >
...
export default {
  ...
  mounted() {
    ...
    eventBus.$on("toExchange0", function(data0) {
      ...
      that.newData =
        Data.now_price != undefined ? Data.now_price : data0.now_price; //初始最新价赋值
    ...
    connect() {
      ...
      that.$socket.on("transaction", msg => {
        ...
        if (msg.type == "transaction") {
          ...
          if (
            msg.legal_id == that.legal_id &&
            msg.currency_id == that.currency_id
          ) {
